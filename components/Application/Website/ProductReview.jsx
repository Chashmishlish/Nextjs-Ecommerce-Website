...
const ProductReview = ({ productId }) => {
    ...
    const [rating, setRating] = useState(0); // added for stars
    ...
    const handleReviewSubmit = async (values) => {
        if (!auth?._id) {
            showToast("error", "Please login first to submit review");
            return;
        }

        if (values.rating === 0) {
            showToast("error", "Please select stars before submitting review");
            return;
        }
    ...
    const { error, data, isFetching, fetchNextPage, hasNextPage } = useInfiniteQuery({
        ...
        queryKey: ['product-review', productId], // ✅ productId add karo
        ...
        <div className="shadow rounded border mb-20">
            ...
            <div className="p-5">
                <div className="flex justify-between flex-wrap items-center">
                    <div className="md:w-1/2 w-full md:flex md:gap-10 md:mb-5">
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        ...
                                                        <FormControl>
                                                            <Textarea placeholder="Please write your comment here..." {...field} />
                                                        </FormControl>
                                                        <FormMessage />
                                                    </FormItem>
                                                )}
